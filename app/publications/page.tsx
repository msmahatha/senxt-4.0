  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white pt-32 pb-24 px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        <div className="mb-16">
          <h1 className="text-5xl md:text-7xl font-bold tracking-tighter mb-4">{content.headers.publications.title}</h1>
          <p className="text-neutral-400 text-lg md:text-xl max-w-2xl font-light">{content.headers.publications.description}</p>
        </div>

        <div className="flex flex-col gap-8">
          {PUBLICATIONS.map((pub, index) => (
            <div key={`${pub.title}-${index}`} className="reveal-card group relative p-8 md:p-10 rounded-2xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-all duration-500 overflow-hidden" style={{ "--reveal-index": index } as React.CSSProperties}>
